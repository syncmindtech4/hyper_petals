...
import logo from "@/assets/luxe-floral-mark.png.asset.json";
    ...
    <footer className="mt-24 border-t border-border/60 bg-[oklch(0.955_0.012_60)]">
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-16 md:grid-cols-4">
        <div className="md:col-span-2">
          <p className="font-serif text-2xl text-foreground">{site.full}</p>
          ...
          <div className="mt-5 flex items-center gap-4 text-foreground/70">
            <a href={site.instagram} aria-label="Instagram" className="transition-colors hover:text-primary"><Instagram className="h-4 w-4" /></a>
            <a href={site.facebook} aria-label="Facebook" className="transition-colors hover:text-primary"><Facebook className="h-4 w-4" /></a>
          </div>
        </div>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3 text-sm text-foreground/80">
            <li className="flex items-start gap-2"><MapPin className="mt-0.5 h-4 w-4 text-primary" /><span>{site.address}</span></li>
          ...
          <p className="mt-4 text-xs uppercase tracking-[0.2em] text-muted-foreground">{site.hours}</p>
        </div>
